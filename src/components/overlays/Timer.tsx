...
export function Timer({ config: c, overlayId }: TimerProps) {
  ...
  useWebSocket({
    ...
    onMessage: (msg) => {
      if (msg.type === 'command') {
        ...
        if (msg.event === 'timer:start') {
          start();
        } else if (msg.event === 'timer:pause') {
          pause();
        } else if (msg.event === 'timer:reset') {
          const data = msg.data as { minutes?: number; seconds?: number };
          reset(data.minutes, data.seconds);
        }
      }
    },
  });
  ...
  if (isCircular) {
    ...
    const elapsed = total - (cfg.mode === 'countdown' ? (total - 0) : 0);
